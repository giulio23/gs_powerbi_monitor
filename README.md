...
- `Tenant.Read.All` *(required for Admin API access)*
...
**Important:**
- You must grant **admin consent** for all permissions above.
- As of 2024, Microsoft restricts some Admin API permissions for new app registrations. If you do not see `Tenant.Read.All`, ensure your tenant allows Power BI service principal access and that your app registration is not restricted by conditional access or security defaults.
- The service principal must be allowed in the Power BI Admin Portal under **Tenant settings > Admin API settings**. Enable "Allow service principals to use Power BI APIs" and add your app registration if required.
- The service principal must be granted access to each workspace (either directly or via a security group).
...
### 🔐 Service Principal & Admin API Permissions

To use the Power BI Admin API with a service principal, your Azure AD app registration and tenant must meet these requirements:

1. **Application Permissions** (not delegated):
   - `Tenant.Read.All` *(required for Admin API access)*
   - `Dataset.Read.All`, `Dataset.ReadWrite.All`
   - `Dataflow.Read.All`, `Dataflow.ReadWrite.All`
   - `Group.Read.All`, `Workspace.Read.All`
2. **Admin Consent**: All permissions must be granted admin consent by an Azure AD administrator.
3. **Power BI Admin Portal Settings**:
   - In the Power BI Admin Portal, under **Tenant settings > Admin API settings**, enable "Allow service principals to use Power BI APIs" and add your app registration or security group.
   - If you do not see the required permissions, your tenant may be restricted by Microsoft. Use an existing app registration with legacy permissions if possible.
4. **Workspace Access**: The service principal must be granted access to each workspace (directly or via a security group).
5. **Known Limitations**:
   - As of 2024, Microsoft restricts new app registrations from receiving some Admin API permissions. If you encounter 401/403 errors, verify your app registration and tenant settings, or use an older app registration if available.
   - The Admin API requires the `$top` parameter for some endpoints (e.g., `/admin/groups?$top=100`).

**References:**
- [Power BI Service Principal documentation](https://learn.microsoft.com/power-bi/developer/embedded/embed-service-principal)
- [Power BI Admin API permissions](https://learn.microsoft.com/power-bi/developer/automation/service-principal)
- [Power BI Admin Portal settings](https://learn.microsoft.com/power-bi/admin/service-security)
